...
document.querySelector('.navigation__button__books').addEventListener('click', function (event) {
  event.preventDefault(); // Останавливаем дефолтное поведение
...
document.querySelector('.chapters__header__link-closed').addEventListener('click', function (event) {
  event.preventDefault(); // Останавливаем дефолтное поведение
  const content = document.querySelector('.navigation__content'); // Элемент, который нужно изменить
...
allLinks.forEach(link => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Находим кнопки для показа формы и саму форму
    const commentButtons = document.querySelectorAll('.comments__buttons button:first-child');
    const forms = document.querySelectorAll('.comment__form');
    const peopleResponses = document.querySelectorAll('.people__comments');
  
    commentButtons.forEach((button, index) => {
      button.addEventListener('click', () => {
        const form = forms[index]; // Выбираем соответствующую форму
        const peopleResponse = peopleResponses[index]; // Выбираем соответствующую форму
        const chapterElement = button.closest('.books'); // Ищем блок с главой
        const chapterName = chapterElement.dataset.chapter; // Берем название главы из data-атрибута
  
        // Показать форму с анимацией
        form.classList.toggle('active');
        peopleResponse.classList.toggle('people-active');
  
        // Устанавливаем значение в select
        const select = form.querySelector('select');
        if (select) {
          [...select.options].forEach(option => {
            option.selected = option.textContent === chapterName;
          });
        }
      });
    });
